...
assessor.load_weights(sorted(glob.glob(os.path.join(this_assessor_save_dir, "*_epoch*.hdf5")))[-1])

# Use very less learning rate
optimizer = Adam(lr=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=1e-3)

assessor.compile(optimizer=optimizer, loss=loss, metrics=['accuracy'])

this_assessor_save_dir = "/".join(this_assessor_save_dir.split("/")[:-1] + ["_".join(this_assessor_save_dir.split("/")[-1].split("_")[:2] + ["FINETUNE"] + this_assessor_save_dir.split("/")[-1].split("_")[2:])])
