...
optimizer = Adam(lr=5e-5, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=adam_decay)

assessor.compile(optimizer=optimizer, loss=loss, metrics=['accuracy'])

this_assessor_save_dir = "/".join(this_assessor_save_dir.split("/")[:-1] + ["_".join(this_assessor_save_dir.split("/")[-1].split("_")[:2] + ["FINETUNE2"] + this_assessor_save_dir.split("/")[-1].split("_")[2:])])
