...
#######################################################
# SYNCNET_TRAIN
syncnet_spl_filenames_train = []
# Train from LRW_TRAIN:
offset = 0
samples_per_word = 400
for w in range(500):
    for i in range(samples_per_word):
        syncnet_spl_filenames_train.append(lrw_word_set_num_txt_file_names_by_word_train[w][offset + i])
...
for w in range(500):
    for i in range(samples_per_word):
        syncnet_spl_filenames_train.append(lrw_word_set_num_txt_file_names_by_word_val[w][offset + i])
...
history = parallel_model.fit_generator(train_generator, steps_per_epoch=train_steps, epochs=epochs, verbose=1,
...
val_generator = vikram_generator(spl_filenames_val, batch_size=batch_size, shuffle=False, random_crop=False, random_flip=False)
val_steps = len((spl_filenames_val)) // batch_size
val_preds = np.empty((0, 500))
for i in tqdm.tqdm(range(val_steps)):
    (X, y) = next(val_generator)
    preds = parallel_model.predict(X)
    val_preds = np.vstack((val_preds, preds))

parallel_model.save_weights(os.path.join(save_dir, "final_parallel_weights_tl0.6845_ta0.7954.hdf5"))
...
k = vikram_generator(spl_filenames_train, batch_size=batch_size, skip=to_skip, shuffle=False, random_crop=False, random_flip=False)
...
if os.path.exists(npz_name_val+".npz"):
    ...
    valY = npz_val["valY"]
    to_skip = len(valDense)//batch_size
...
k = vikram_generator(spl_filenames_val, batch_size=batch_size, skip=to_skip, shuffle=False, random_crop=False, random_flip=False)
...
for i in tqdm.tqdm(range(to_skip, n_steps), initial=to_skip, total=n_steps):
    ...
    if (i+1)%10 == 0:
        np.savez(npz_name_val, valDense=valDense, valSoftmax=valSoftmax, valY=valY)
...
if os.path.exists(npz_name_test+".npz"):
    ...
    to_skip = len(testDense)//batch_size
...
for i in tqdm.tqdm(range(to_skip, n_steps), initial=to_skip, total=n_steps):
    ...
    if (i+1)%10 == 0:
        np.savez(npz_name_test, testDense=testDense, testSoftmax=testSoftmax, testY=testY)
